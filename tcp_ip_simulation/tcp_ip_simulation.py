...
from tcp_ip_simulation.tcp_ip_node import TcpIpNode, AddressSpaceFullError
...
class TcpIpSimulation:
    "Creates docmuentation for a TCP/IP simulation with any number of students."
    ...
    def __init__(self, participants, outfile="simulation.html",
        ...
        self.env = Environment(loader=PackageLoader("tcp_ip_simulation"))
    ...
    def generate_instructions(self):
        template = self.env.get_template('simulation.html')
        nodes = []
        ...
        networks = sorted(set(
            [n.get_network_name() for n in self.rootNode._get_nodes(recursive=True)]
        ))
        network_list = strlist(networks[1:]) if len(networks) > 1 else ''
        sender = nodes[-2]
        for node in nodes:
            if node.get('address') == sender.get('nameserver'):
                sender_nameserver = node
                break
        recipient = nodes[-1]
        with open(self.outfile, 'w') as outfile:
            outfile.write(template.render({
                'tree': self.rootNode.tree_string(with_root=False),
                'network_0': strlist([n.name for n in self.rootNode._get_nodes()]),
                'network_list': network_list,
                'nodes': nodes,
                'n': len(nodes),
                'sender': sender, 
                'sender_nameserver': sender_nameserver,
                'recipient': recipient,
                'directory': sorted(self._get_directory().items()),
            }))
    ...
    # Each node will have a nameserver who provides IP address lookups.
...
def strlist(l):
    if not l: return ''
    rest, tail = l[:-1], l[-1]
    parts = [', '.join(rest), tail] if rest else [tail]
